...
-- How you open the bank menu with pressing E then esc to exit
    if canBank then
        Citizen.CreateThread(function()
      while true do
        Wait(0)
        if atBank() or atATM() then
...
-- looks to see if you are at the bank
function atBank()
...
-- looks to see if you are at the ATM
function atATM()
	local player = GetPlayerPed(-1)
	local playerloc = GetEntityCoords(player, 0)
